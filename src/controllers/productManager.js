...
class ProductManager {
  ...
  async updateProduct(id, changes) {
    try {
      ...
      const codeExist = this.products.some(
        (item) => item.code === changes.code
      );
      if (codeExist) {
        throw new Error("The code already exists");
      }

      if (index !== -1 || !codeExist) {
  ...
  async updateProductByCode(changes, code) {
    try {
      const index = this.products.findIndex((item) => item.code === code);
      if (index !== -1) {
        if (
          changes.code &&
          this.products.some(
            (item, i) => i !== index && item.code === changes.code
          )
        ) {
          throw new Error("The code already exists");
        }

        this.products[index] = { ...this.products[index], ...changes, code };
      } else {
  ...
  async deleteProduct(id) {
    try {
      ...
      if (index !== -1) {
        ...
        console.log("Couldn't delete product, does not exist");
      }
    } catch (error) {
